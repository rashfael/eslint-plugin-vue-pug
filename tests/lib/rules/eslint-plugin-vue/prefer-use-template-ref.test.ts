// AUTOGENERATED FROM https://github.com/vuejs/eslint-plugin-vue/blob/3868a2d6e7b44037dd5aa04a7acdd857a0358a7b/tests/lib/rules/prefer-use-template-ref.test.ts
/**
 * @author Thomasan1999
 * See LICENSE file in root directory for full license.
 */
import { RuleTester } from '../../../eslint-compat'
import rule from 'eslint-plugin-vue/dist/rules/prefer-use-template-ref'
import vueEslintParser from 'vue-eslint-parser'

const tester = new RuleTester({
  languageOptions: {
    parser: vueEslintParser,
    ecmaVersion: 2020,
    sourceType: 'module',
    parserOptions: {
      templateTokenizer: { pug: 'vue-eslint-parser-template-tokenizer-pug' }
    }
  }
})

tester.run('prefer-use-template-ref', rule, {
  valid: [
    {
      filename: 'single-use-template-ref.vue',
      code: `
      <template lang="pug">div(ref="root")</template>
      <script setup>
        import { useTemplateRef } from 'vue';
        const root = useTemplateRef('root');
      </script>`
    },
    {
      filename: 'multiple-use-template-refs.vue',
      code: `
      <template lang="pug">
button(ref="button") Content
a(href="", ref="link") Link
</template>
      <script setup>
        import { useTemplateRef } from 'vue';
        const buttonRef = useTemplateRef('button');
        const link = useTemplateRef('link');
      </script>`
    },
    {
      filename: 'use-template-ref-in-block.vue',
      code: `
      <template lang="pug">
div
  ul
    li(ref="firstListItem") Morning
    li Afternoon
    li Evening
</template>
      <script setup>
        import { useTemplateRef } from 'vue';
        function getFirstListItemElement() {
          const firstListItem = useTemplateRef('firstListItem')
          console.log(firstListItem)
        }
      </script>`
    },
    {
      filename: 'non-template-ref.vue',
      code: `
      <template lang="pug">
div
  ul
    li(v-for="food in foods", :key="food") {{food}}
</template>
      <script setup>
        import { ref } from 'vue';
        const foods = ref(['Spaghetti', 'Pizza', 'Cake']);
      </script>`
    },
    {
      filename: 'counter.js',
      code: `
      import { ref } from 'vue';
      const counter = ref(0);
      const names = ref(new Set());
      function incrementCounter() {
        counter.value++;
        return counter.value;
      }
      function storeName(name) {
        names.value.add(name)
      }
      `
    },
    {
      filename: 'setup-function.vue',
      code: `
      <template lang="pug">
p Button clicked
  | {{counter}}
  | times.
button(ref="button", @click="counter++") Click
</template>
      <script>
        import { ref, useTemplateRef } from 'vue';
        export default {
          name: 'Counter',
          setup() {
            const counter = ref(0);
            const button = useTemplateRef('button');
          }
        }
      </script>`
    },
    {
      filename: 'options-api-no-refs.vue',
      code: `<!-- CONVERT ERROR -->Element is missing end tag.
<!-- CONVERT ERROR -->Element is missing end tag.      <template>
        <label ref="label">
          Name:
          <input v-model="name" />
        </label>
        <p ref="textRef">{{niceName}}</p>
        <button
      </template>
      <script>
        import { ref } from 'vue';
        export default {
          name: 'NameRow',
          methods: {
            someFunction() {
              return {
                label: ref(5),
              }
            }
          }
          data() {
            return {
              label: 'label',
              name: ''
            }
          },
          computed: {
            niceName() {
              return 'Nice ' + this.name;
            }
          }
        }
      </script>
      `
    },
    {
      filename: 'options-api-mixed.vue',
      code: `
      <template lang="pug">
label(ref="labelElem") Name:
  input(v-model="name")

| {{ loremIpsum }}
</template>
      <script>
        import { ref } from 'vue';
        export default {
          name: 'NameRow',
          props: {
            defaultLabel: {
              type: String,
            },
          },
          data() {
            return {
              label: ref(this.defaultLabel),
              labelElem: ref(),
              name: ''
            }
          },
          computed: {
            loremIpsum() {
              return this.name + ' lorem ipsum'
            }
          }
        }
      </script>`
    },
    {
      filename: 'template-ref-function.vue',
      code: `
      <template lang="pug">button(:ref="ref => button = ref") Content</template>
      <script setup>
        import { ref } from 'vue';
        const button = ref();
        </script>`
    },
    {
      filename: 'ref-in-block.vue',
      code: `
      <template lang="pug">
div
  ul
    li(ref="firstListItem") Morning
    li(ref="second") Afternoon
    li Evening
</template>
      <script setup>
        import { ref, shallowRef } from 'vue';
        function getFirstListItemElement() {
          const firstListItem = ref();
          const nestedCallback = () => {
            const second = shallowRef();
            console.log(second);
          }
          nestedCallback();
        }
      </script>`
    },
    {
      filename: 'ref-in-block-setup-fn.vue',
      code: `
      <template lang="pug">
div
  ul
    li(ref="firstListItem") Morning
    li(ref="second") Afternoon
    li Evening
</template>
      <script>
        import { ref, shallowRef } from 'vue';
        export default {
          name: 'ComponentWithRefInBlock',
          setup() {
            function getFirstListItemElement() {
              const firstListItem = shallowRef();
              const nestedCallback = () => {
                const second = ref();
                console.log(second);
              }
              nestedCallback();
            }
          }
        }
      </script>`
    },
    {
      filename: 'non-block-arrow-setup-function.vue',
      code: `
        <script>
          import { defineComponent } from 'vue';
          export default defineComponent({
            setup: () => ({})
          })
        </script>
      `
    },
    {
      filename: 'multiple-scripts-setup-first.vue',
      code: `
      <template lang="pug">div(ref="root", :data-a="A")</template>

      <script setup>
      import { useTemplateRef } from 'vue'
      const root = useTemplateRef('root')
      </script>

      <script>
      const A = 'foo'
      </script>`
    },
    {
      filename: 'multiple-scripts-setup-last.vue',
      code: `
      <template lang="pug">div(ref="root", :data-a="A")</template>

      <script>
      const A = 'foo'
      </script>

      <script setup>
      import { useTemplateRef } from 'vue'
      const root = useTemplateRef('root')
      </script>`
    }
  ],
  invalid: [
    {
      filename: 'single-ref.vue',
      code: `
      <template lang="pug">div(ref="root")</template>
      <script setup>
        import { ref } from 'vue';
        const root = ref();
      </script>`,
      errors: [
        {
          messageId: 'preferUseTemplateRef',
          data: {
            name: 'ref'
          },
          line: 7,
          column: 22,
          endLine: 7,
          endColumn: 27
        }
      ]
    },
    {
      filename: 'one-ref-unused-in-script.vue',
      code: `
      <template lang="pug">
button(ref="button") Content
a(href="", ref="link") Link
</template>
      <script setup>
        import { ref } from 'vue';
        const buttonRef = ref();
        const link = ref();
      </script>`,
      errors: [
        {
          messageId: 'preferUseTemplateRef',
          data: {
            name: 'ref'
          },
          line: 9,
          column: 22,
          endLine: 9,
          endColumn: 27
        }
      ]
    },
    {
      filename: 'multiple-refs.vue',
      code: `
      <template lang="pug">
h1(ref="heading") Heading
a(href="", ref="link") Link
</template>
      <script setup>
        import { ref } from 'vue';
        const heading = ref();
        const link = ref();
      </script>`,
      errors: [
        {
          messageId: 'preferUseTemplateRef',
          data: {
            name: 'ref'
          },
          line: 8,
          column: 25,
          endLine: 8,
          endColumn: 30
        },
        {
          messageId: 'preferUseTemplateRef',
          data: {
            name: 'ref'
          },
          line: 9,
          column: 22,
          endLine: 9,
          endColumn: 27
        }
      ]
    },
    {
      filename: 'setup-function-only-refs.vue',
      code: `
      <template lang="pug">
p Button clicked
  | {{counter}}
  | times.
button(ref="button") Click
</template>
      <script>
        import { ref } from 'vue';
        export default {
          name: 'Counter',
          setup() {
            const counter = ref(0);
            const button = ref();
          }
        }
      </script>`,
      errors: [
        {
          messageId: 'preferUseTemplateRef',
          data: {
            name: 'ref'
          },
          line: 12,
          column: 28,
          endLine: 12,
          endColumn: 33
        }
      ]
    },
    {
      filename: 'single-shallowRef.vue',
      code: `
      <template lang="pug">div(ref="root")</template>
      <script setup>
        import { shallowRef } from 'vue';
        const root = shallowRef();
      </script>`,
      errors: [
        {
          messageId: 'preferUseTemplateRef',
          data: {
            name: 'shallowRef'
          },
          line: 7,
          column: 22,
          endLine: 7,
          endColumn: 34
        }
      ]
    },
    {
      filename: 'block-arrow-setup-function.vue',
      code: `
      <template lang="pug">button(ref="button") Click</template>
      <script>
        import { ref } from 'vue';
        export default {
          setup: () => {
            const button = ref();
          }
        }
      </script>`,
      errors: [
        {
          messageId: 'preferUseTemplateRef',
          data: {
            name: 'ref'
          },
          line: 9,
          column: 28,
          endLine: 9,
          endColumn: 33
        }
      ]
    },
    {
      filename: 'multiple-scripts-setup-first.vue',
      code: `
      <template lang="pug">div(ref="root", :data-a="A")</template>

      <script setup>
      import { ref } from 'vue'
      const root = ref()
      </script>

      <script>
      const A = 'foo'
      </script>`,
      errors: [
        {
          messageId: 'preferUseTemplateRef',
          data: {
            name: 'ref'
          },
          line: 8,
          column: 20,
          endLine: 8,
          endColumn: 25
        }
      ]
    },
    {
      filename: 'multiple-scripts-setup-last.vue',
      code: `
      <template lang="pug">div(ref="root", :data-a="A")</template>

      <script>
      const A = 'foo'
      </script>

      <script setup>
      import { ref } from 'vue'
      const root = ref()
      </script>`,
      errors: [
        {
          messageId: 'preferUseTemplateRef',
          data: {
            name: 'ref'
          },
          line: 12,
          column: 20,
          endLine: 12,
          endColumn: 25
        }
      ]
    }
  ]
})
